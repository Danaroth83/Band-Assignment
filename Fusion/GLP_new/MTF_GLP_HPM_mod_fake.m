...
function I_Fus_MTF_GLP_HPM = MTF_GLP_HPM_mod_fake(I_PAN,I_MS,I_GT,GNyq,ratio)
% keyboard
...
% std2_imageHR_LR=std2(imresize(imresize(imageHR,1/ratio),ratio));
imageHR = repmat(imageHR,[1 1 size(I_MS,3)]);

for ii = 1 : size(I_MS,3)
    imageHR(:,:,ii) = (imageHR(:,:,ii) - mean2_imageHR).*(std2(I_GT(:,:,ii))./std2_imageHR) + mean2(I_GT(:,:,ii));
    % imageHR(:,:,ii) = (imageHR(:,:,ii) - mean2_imageHR).*(std2(I_MS(:,:,ii))./std2_imageHR_LR) + mean2(I_MS(:,:,ii));  
end
