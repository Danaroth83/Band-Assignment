...
% maxI_MS=max(I_MS(:));
% imageHR = double(I_PAN)/max(I_PAN(:));
% imageLR = double(I_MS)/maxI_MS;
% maxI_MS=max(I_MS(:));
% imageHR = uint16(I_PAN);
% imageLR = uint16(I_MS);
...
for ii = 1 : size(imageLR,3)  
  muhr1=  mean2(imageHR(:,:,ii))
  mulr1=  mean2(imageLR(:,:,ii))
  minhr1=min(min(imageHR(:,:,ii)))
%  c=min(std2(imageLR(:,:,ii))./std2(imageHR(:,:,ii)),1)
  c=std2(imageLR(:,:,ii))./std2(imageHR(:,:,ii))
 imageHR(:,:,ii) = (imageHR(:,:,ii) - mean2(imageHR(:,:,ii))).*(std2(imageLR(:,:,ii))./std2(imageHR(:,:,ii))) + mean2(imageLR(:,:,ii));  
%   imageHR(:,:,ii) = (imageHR(:,:,ii) - mean2(imageHR(:,:,ii)))+ mean2(imageLR(:,:,ii));  
%    imageHR(:,:,ii) = imageHR(:,:,ii)./mean2(imageHR(:,:,ii)).*mean2(imageLR(:,:,ii)); 
%    imageHR(:,:,ii) = (imageHR(:,:,ii) - mean2(imageHR(:,:,ii))).*c + mean2(imageLR(:,:,ii));  
%     imageHR(:,:,ii) = imageHR(:,:,ii) .*(std2(imageLR(:,:,ii))./std2(imageHR(:,:,ii)));  
%  imageHR(:,:,ii) = (imageHR(:,:,ii) - mean2(imageHR(:,:,ii))).*(std2(imageLR(:,:,ii))./std2(imageHR(:,:,ii))) ;  

muhr2= mean2(imageHR(:,:,ii))
minhr2=min(min(imageHR(:,:,ii)))
end
imageHR=max(imageHR,0);
imageHR = double(imageHR);
imageLR = double(imageLR);
% cd Equalizzazione
% for ii = 1 : size(imageLR,3)
...
%             GNyq = 0.15 .* ones(1,8);
%         else
%             GNyq = 0.29 .* ones(1,size(imageLR,3)); %toolbox
% %             GNyq = 0.3 .* ones(1,size(imageLR,3));
%         end
% end

%%% MTF
N = 41;
PAN_LP = zeros(size(imageLR));
nBands = size(imageLR,3);
fcut = 1/ratio;
...
% I_Fus_MTF_GLP_HPM = imageLR .* (imageHR ./ (PAN_LP + eps))*maxI_MS;
I_Fus_MTF_GLP_HPM = imageLR .* (imageHR ./ (PAN_LP + eps));
end