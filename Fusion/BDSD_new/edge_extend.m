...
pix_extend=repmat(pix_extend,3-size(pix_extend));
...
for ii=1:Nd
    I_out(pixu+1:end-pixd,pixl+1:end-pixr,ii)=I_in(:,:,ii);
    I_out([1:pixu,end-pixd+1:end],pixl+1:end-pixr,ii)=...
        flipud(I_in([end-pixd+1-K:end-K,1+K:pixu+K],:,ii));
    I_out(pixu+1:end-pixd,[1:pixl, end-pixr+1:end],ii)=...
        fliplr(I_in(:,[end-pixr+1-K:end-K,1+K:pixl+K],ii));
    I_out([1:pixu,end-pixd+1:end],[1:pixl,end-pixr+1:end],ii)=rot90(I_in(...
        [end-pixd+1-K:end-K,1+K:pixu+K],[end-pixr+1-K:end-K,1+K:pixl+K],ii),2);
end
