function Out = CNMF_init(xdata,ydata,w,M,hyper,multi,delta,I_in,delta_h,delta_m,srf,mask)

%--------------------------------------------------------------------------
% COUPLED NONNEGATIVE MATRIX FACTORIZATION (CNMF)
% 
% This is an initilization function of CNMF.
%
% USAGE
...
%[U, indicies, snrEstimate] = hyperVca(hyper, M); % initialize W_hyper: (band, M)
[~, indicies] = vca(hyper, M);
W_hyper = hyper(:,indicies);
...
switch masking
    ...
    case 1
        ...
        %H_hyper = hyperFcls( hyper, W_hyper );
end
