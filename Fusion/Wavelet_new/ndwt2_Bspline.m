function varargout = ndwt2_Bspline(X,ratio,order)
...
for k=1:level
    st_ones=ones(1,fac(k));
    LoD=st_ones;
    for ii=1:order
        LoD=conv(LoD,st_ones);
    end
    LoD=sqrt(2)/fac(k)^(order+1)*LoD;
    dec = decFUNC(X,LoD,dwtEXTM);
    X = dec{1,1,1};
...
%-------------------------------------------------------------------------%
function dec = decFUNC(X,LoD,dwtEXTM)

dec = cell(2,2);
permVect = [];
[a_Lo] = wdec1D(X,LoD{1},permVect,dwtEXTM);
permVect = [2,1,3];
[dec{1,1},dec{1,2}] = wdec1D(a_Lo,LoD{2},permVect,dwtEXTM);
[dec{2,1},dec{2,2}] = wdec1D(d_Hi,LoD{2},permVect,dwtEXTM);

%-------------------------------------------------------------------------%
function L = wdec1D(X,Lo,perm,dwtEXTM)
